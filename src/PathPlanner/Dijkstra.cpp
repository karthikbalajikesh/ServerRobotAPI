...
Dijkstra::Dijkstra(pair<float,float> destination, int numIterations, int zmax, int xmax, int cellWidth, int cellHeight, int neighbours, cood_vector coordinates ) :
...
void Dijkstra::addDestination(pair<float, float>& dest) {
...
vector<GraphNode> Dijkstra::shortestPath() {
...
void Dijkstra::relaxNode(Queue& minHeap, int& currentIndex, float& currentDistance,
...
vector<GraphNode> Dijkstra::retrievePath(vector<int>& previousNode) {
	vector<GraphNode> Path;
...
void Dijkstra::PrintPathDetails() {
